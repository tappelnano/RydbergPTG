...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 14 15:20:18 2023
...
def P_p (N):
    ket_1 = basis(2)
    ket_0 = basis(2,1)
    P = ket_0 * ket_1.dag() + ket_1 * ket_0.dag()
    for i in range (1,N):
        P = tensor(P,ket_0 * ket_1.dag() + ket_1 * ket_0.dag())
    return P
"""     
def H_right (N, L, k):
...
def rel_error_plot (L):
    ...
    for l in L:
        ...
        diff2 = Qobj(np.array((H_right(3, l, 3)))) - (H_eff(2**3,l))
        norm_vals.append(op_norm(diff) / np.linalg.norm(Qobj(np.array(H_right(3, l, 3))),2))
        norm_vals2.append(op_norm(diff2 / np.linalg.norm(Qobj(np.array(H_right(3, l, 3))))))
    plt.plot(L, norm_vals, label = "f = 31 \u03BB$^3$ + 0.92 \u03BB$^2$ + 0.003 \u03BB")
    plt.plot(L, norm_vals2,  label = "f = 0")
    ax = plt.subplot(111)
    ax.spines['right'].set_color((.8,.8,.8))
    ax.spines['top'].set_color((.8,.8,.8))
    #plt.title("X Perturbation: 2-norm error", fontsize=16)
...
def rel_error_bounds (L):
    ...
    ax.spines['right'].set_color((.8,.8,.8))
    ax.spines['top'].set_color((.8,.8,.8))
    plt.title("X Perturbation: 2-norm error bounds", fontsize=16)
    plt.xlabel('Perturbation strength \u03BB', fontsize=14)
